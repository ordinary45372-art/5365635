...
async function loadFailedResets() {
  const data = await storeGet('pool_failed_resets', []);
  return Array.isArray(data) ? data : [];
}
async function saveFailedResets(list) {
  await storeSet('pool_failed_resets', Array.isArray(list) ? list : []);
}
async function pushFailedReset(entry) {
  const list = await loadFailedResets();
  const id = entry.id || crypto.randomUUID();
  const row = Object.assign({}, entry, {
    id,
    _failedAt: Date.now(),
    _failError: entry._failError || entry.error || 'reset failed',
    _rateLimited: !!entry._rateLimited,
  });
  // de-dupe by cookie/username
  const key = (row.cookie || '') + '|' + (row.username || row.user || '');
  const filtered = list.filter((x) => ((x.cookie || '') + '|' + (x.username || x.user || '')) !== key);
  filtered.push(row);
  await saveFailedResets(filtered);
  return row;
}
...
async function preparePoolAccount(data, type) {
  ...
  if (!ch.ok) {
    console.log('[pool] password reset failed (kept for staff retry):', ch.error || 'unknown');
    await pushFailedReset({
      id: data.id || crypto.randomUUID(),
      username: data.username || data.user || data.pseudo || '',
      user: data.user || data.username || '',
      password: oldPass,
      cookie,
      type,
      _poolType: type,
      _failError: ch.error || 'reset failed',
      _rateLimited: !!ch.rateLimited,
    });
    return null;
  }
...
app.post('/api/roblox/friend-request', async (req, res) => {
  ...
  const userId = b.targetUserId || b.userId;
  ...
  try {
    const r = await authPost(
      ...
      { friendshipOriginSourceType: 'UserProfile' },
      cookie
    );
    if (r.status === 429) return res.json({ ok: false, rateLimited: true });
    const j = await r.json().catch(() => null);
    if (j && j.isCaptchaRequired) return res.json({ ok: false, captcha: true });
    if (r.ok && (j == null || j.success !== false)) return res.json({ ok: true, data: j });
    return res.json({
      ok: false,
      error: (j && j.errors && j.errors[0] && j.errors[0].message) || ('HTTP ' + r.status),
      data: j,
...
app.post('/api/roblox/follow-user', async (req, res) => {
  ...
  try {
    ...
    const j = await r.json().catch(() => null);
    if (j && j.isCaptchaRequired) return res.json({ ok: false, captcha: true });
    if (r.ok) return res.json({ ok: true, data: j });
    return res.json({
      ok: false,
      error: (j && j.errors && j.errors[0] && j.errors[0].message) || ('HTTP ' + r.status),
      data: j,
...
// Desktop login pairing (Mongo so it works across Render instances)
app.post('/api/auth/desktop-pair', async (req, res) => {
  try {
    const u = authUser(req);
    if (!u || !u.uid || u.role === 'staff') return res.json({ ok: false, error: 'Sign in first' });
    const device = String((req.body && req.body.device) || '').trim().slice(0, 80);
    if (!device) return res.json({ ok: false, error: 'Missing device' });
    const h = req.headers.authorization || '';
    const token = h.startsWith('Bearer ') ? h.slice(7) : '';
    if (!token) return res.json({ ok: false, error: 'No session' });
    const db = await getDb();
    if (!db) return res.json({ ok: false, error: 'Database offline' });
    const col = db.collection('desktop_pair');
    await col.updateOne(
      { device },
      { $set: { device, token, exp: new Date(Date.now() + 10 * 60 * 1000) } },
      { upsert: true }
    );
    res.json({ ok: true });
  } catch (e) {
...
app.post('/api/auth/desktop-wait', async (req, res) => {
  try {
    const device = String((req.body && req.body.device) || '').trim();
    if (!device) return res.json({ ok: false, error: 'Missing device' });
    const db = await getDb();
    if (!db) return res.json({ ok: false, pending: true });
    const col = db.collection('desktop_pair');
    const entry = await col.findOne({ device });
    if (!entry) return res.json({ ok: false, pending: true });
    if (entry.exp && new Date(entry.exp).getTime() < Date.now()) {
      await col.deleteOne({ device });
...
app.post('/api/staff/stock', async (req, res) => {
  const u = authUser(req);
  if (!u || u.role !== 'staff') return res.json({ ok: false, error: 'Forbidden' });
  try {
    const pool = await loadPool();
    const failed = await loadFailedResets();
    const byType = {};
    for (const a of pool) {
      const t = a._poolType || a.type || 'unknown';
    ...
    res.json({
      ...
      stock: pool.map((a) => ({
        id: a.id,
        username: a.username || a.user || a.pseudo || '—',
        type: a._poolType || a.type || '—',
        password: a.password || '',
        pooledAt: a._pooledAt || null,
      })),
      failed: failed.map((a) => ({
        id: a.id,
        username: a.username || a.user || '—',
        type: a._poolType || a.type || '—',
        error: a._failError || 'reset failed',
        rateLimited: !!a._rateLimited,
        failedAt: a._failedAt || null,
      })),
    });
  } catch (e) {
    res.json({ ok: false, error: e.message });
...
app.post('/api/staff/failed-delete', async (req, res) => {
  const u = authUser(req);
  if (!u || u.role !== 'staff') return res.json({ ok: false, error: 'Forbidden' });
  try {
    const { id, all } = req.body || {};
    let failed = await loadFailedResets();
    if (all) {
      const n = failed.length;
      await saveFailedResets([]);
      return res.json({ ok: true, deleted: n });
    }
    if (!id) return res.json({ ok: false, error: 'Missing id' });
    const before = failed.length;
    failed = failed.filter((x) => String(x.id) !== String(id));
    await saveFailedResets(failed);
    res.json({ ok: true, deleted: before - failed.length });
  } catch (e) {
    res.json({ ok: false, error: e.message });
  }
});

app.post('/api/staff/retry-reset', async (req, res) => {
  const u = authUser(req);
  if (!u || u.role !== 'staff') return res.json({ ok: false, error: 'Forbidden' });
  try {
    const { id } = req.body || {};
    if (!id) return res.json({ ok: false, error: 'Missing id' });
    let failed = await loadFailedResets();
    const idx = failed.findIndex((x) => String(x.id) === String(id));
    if (idx < 0) return res.json({ ok: false, error: 'Not found' });
    const entry = failed[idx];
    const type = entry._poolType || entry.type || 'alt';
    const newPass = makePoolPassword();
    const ch = await changeRobloxPassword(entry.cookie, entry.password, newPass);
    if (!ch.ok) {
      failed[idx] = Object.assign({}, entry, {
        _failError: ch.error || 'reset failed',
        _rateLimited: !!ch.rateLimited,
        _failedAt: Date.now(),
      });
      ...
      return res.json({
        ok: false,
        rateLimited: !!ch.rateLimited,
        error: ch.error || 'reset failed',
      });
    }
    failed.splice(idx, 1);
    await saveFailedResets(failed);
    ...
    pool.push({
      ...
      password: newPass,
      cookie: ch.cookie || entry.cookie,
      type,
      _poolType: type,
      _pooledAt: Date.now(),
    });
    await savePool(pool);
    discordLog('Staff password retry ok', [
      '**staff:** ' + (u.username || 'staff'),
      '**user:** ' + (entry.username || entry.user || '—'),
      '**type:** ' + type,
    ], 0x57F287);
    res.json({ ok: true, username: entry.username || entry.user });
  } catch (e) {
    res.json({ ok: false, error: e.message });
  }
});

let staffRetryBusy = false;
app.post('/api/staff/retry-reset-batch', async (req, res) => {
  const u = authUser(req);
  if (!u || u.role !== 'staff') return res.json({ ok: false, error: 'Forbidden' });
  if (staffRetryBusy) return res.json({ ok: false, error: 'Batch already running' });
  const delayMs = Math.max(5000, Math.min(300000, parseInt((req.body && req.body.delayMs) || '60000', 10) || 60000));
  const all = !!(req.body && req.body.all);
  staffRetryBusy = true;
  // Respond immediately; work continues in background
  res.json({ ok: true, started: true, delayMs });
  (async () => {
    try {
      let failed = await loadFailedResets();
      const ids = all ? failed.map((x) => x.id) : (Array.isArray(req.body && req.body.ids) ? req.body.ids : failed.map((x) => x.id));
      console.log('[staff] sequential retry', ids.length, 'delay', delayMs);
      for (let i = 0; i < ids.length; i++) {
        failed = await loadFailedResets();
        const entry = failed.find((x) => String(x.id) === String(ids[i]));
        if (!entry) continue;
        const type = entry._poolType || entry.type || 'alt';
        const newPass = makePoolPassword();
        const ch = await changeRobloxPassword(entry.cookie, entry.password, newPass);
        if (ch.ok) {
          failed = failed.filter((x) => String(x.id) !== String(entry.id));
          await saveFailedResets(failed);
          const pool = await loadPool();
          pool.push({
            id: entry.id || crypto.randomUUID(),
          ...
          await savePool(pool);
          console.log('[staff] retry ok', entry.username || entry.id);
        } else {
          const fi = failed.findIndex((x) => String(x.id) === String(entry.id));
          if (fi >= 0) {
            failed[fi] = Object.assign({}, entry, {
              _failError: ch.error || 'reset failed',
              _rateLimited: !!ch.rateLimited,
              _failedAt: Date.now(),
            });
          ...
          console.log('[staff] retry fail', entry.username || entry.id, ch.error);
        }
        if (i < ids.length - 1) await new Promise((r) => setTimeout(r, delayMs));
      }
    } catch (e) {
      console.error('[staff] batch retry', e.message);
    } finally {
      staffRetryBusy = false;
    }
  })();
});
