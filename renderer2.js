  ...
  try {
    ...
    box.querySelectorAll('.earn-dc-join').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        // 1) Try claim if Discord already linked and user is in server
        ...
        // 2) Open invite so they can join if needed
        if (invite) window.open(invite, '_blank');
        // 3) Discord OAuth verifies membership (works if already in server too)
        if (r && r.needDiscord) {
          if (typeof toast === 'function') toast('Verify with Discord to confirm you are in the server…');
          const url = '/api/auth/discord/start?serverId=' + encodeURIComponent(id) +
...
(function handleEarnQuery() {
  try {
    ...
    if (earn === 'ok') {
      ...
      if (typeof toast === 'function') toast('You are not in that Discord yet — join, then press JOIN again');
      if (inv) window.open(decodeURIComponent(inv), '_blank');
    } else if (earn === 'linked') {
