...
async function logClientEvent(type, detail) {
  try {
    if (window.api && window.api.logEvent) await window.api.logEvent(type, detail);
    else await fetch('/api/log/event', {
      method: 'POST',
      headers: Object.assign({ 'Content-Type': 'application/json' },
        (window.__authToken || localStorage.getItem('ogalts_token'))
          ? { Authorization: 'Bearer ' + (window.__authToken || localStorage.getItem('ogalts_token')) }
          : {}),
...
function formatCreated(a) {
  // Roblox account creation date (same as original app) — not when generated on ogalts
  const ts = a.created || a.createdAt || null;
  if (!ts) return '—';
  const d = new Date(ts);
  if (isNaN(d.getTime())) return '—';
...
function showScreen(name, opts = {}) {
  ...
  if (name === 'settings') {
    const genNav = document.querySelector('.set-nav-item[data-set="generation"]');
    const keysNav = document.querySelector('.set-nav-item[data-set="keys"]');
    ...
    if (genNav) {
      ...
      document.querySelectorAll('.set-pane').forEach((p) => { p.hidden = p.dataset.set !== 'generation'; });
    }
  }
...
function renderAccounts() {
  ...
  body.innerHTML = pageAccts.map((a) => {
    ...
    <div class="at-row${on ? ' sel' : ''}" data-id="${a.id}">
      ...
      <div class="ac-folder cell-mono tv-mid" style="display:none"></div>
...
function showDetailFromAccount(a, fresh) {
  ...
  $('#gd-sub').textContent = `${typeOf(a)}${a.region ? ' · ' + a.region : ''} · ${fresh ? 'just now' : (a.dateAdded ? new Date(a.dateAdded).toLocaleDateString() : '')}`;
...
const ACC_COLS = [
  ...
  { key: 'folder', label: 'Folder', hide: true },
];
...
function applyAccCols() {
  ...
  for (const c of ACC_COLS) t.classList.toggle('hide-' + c.key, c.key === 'folder' || !accColOn(c.key));
}
...
// toolbar export removed — use bulk Export on selection
// Bulk actions
...
async function finishGoogleLogin(credential, username) {
  ...
  try {
    ...
    if (q.get('desktop_auth') === '1' || q.get('device')) setTimeout(() => { pairDesktopDevice(); sendLoginToDesktopApp(); }, 300);
  } catch (_) {}
  window.__pendingGoogleCred = null;
  const box = document.getElementById('username-pick');
...
async function initGoogleBtn() {
  // Desktop app: never show Google GIS button (use website login instead)
  if (window.ogaltsDesktop && window.ogaltsDesktop.isDesktop) return;
  try {
...
document.addEventListener('click', (e) => {
  const bulk = e.target.closest && e.target.closest('#bulk-delete');
  if (bulk) {
    setTimeout(() => {
      const n = (window.selected && selected.size) || 0;
      logClientEvent('delete', 'bulk delete selection (~' + n + ' selected at click)');
    }, 0);
  }
}, true);
...
/* ===== Report a problem ===== */
window.__reportState = window.__reportState || {}; // accountId -> { status, reportId }

async function fetchReportStatus(accountId) {
  try {
    const r = await window.api.reportStatus(accountId);
    if (r && r.ok && r.status) {
      window.__reportState[accountId] = { status: r.status, reportId: r.reportId, staffNote: r.staffNote || '' };
      return window.__reportState[accountId];
    }
  } catch (_) {}
  return window.__reportState[accountId] || null;
}

function hideAccountSecrets(hide) {
  const info = document.getElementById('gd-info-block');
  const creds = document.getElementById('gd-creds-block');
  if (info) info.style.display = hide ? 'none' : '';
  if (creds) creds.style.display = hide ? 'none' : '';
  const actions = document.querySelectorAll('#gd-login, #gd-open, #gd-report-btn');
  actions.forEach((el) => { if (el) el.style.display = hide ? 'none' : ''; });
}

async function applyReportUi(a) {
  if (!a || !a.id) return;
  const panel = document.getElementById('gd-report-panel');
  const review = document.getElementById('gd-review-panel');
  const msg = document.getElementById('gd-review-msg');
  const moreWrap = document.getElementById('gd-review-more-wrap');
  if (panel) panel.hidden = true;
  const st = await fetchReportStatus(a.id);
  if (!st || !st.status) {
    if (review) review.hidden = true;
    hideAccountSecrets(false);
    const btn = document.getElementById('gd-report-btn');
    if (btn) btn.style.display = '';
    return;
  }
  if (st.status === 'under_review') {
    hideAccountSecrets(true);
    if (review) review.hidden = false;
    if (msg) msg.textContent = 'The account is under review. Come back later for updates.';
    if (moreWrap) moreWrap.hidden = true;
    const ok = document.getElementById('gd-review-ok');
    if (ok) ok.style.display = 'none';
    return;
  }
  if (st.status === 'unable_to_help' || st.status === 'resolved') {
    hideAccountSecrets(true);
    if (review) review.hidden = false;
    if (msg) {
      msg.textContent = st.status === 'unable_to_help'
        ? 'Sadly we are unable to help with this issue. Please click OK below to show the account info again.'
        : 'Issue resolved. Please click OK below to show the account info again.';
    }
    ...
    ok.onclick = () => {
      hideAccountSecrets(false);
      if (review) review.hidden = true;
      // keep status so they don't re-report spam - clear local only
      delete window.__reportState[a.id];
    };
    const sendMore = document.getElementById('gd-review-send-more');
    if (sendMore) {
      sendMore.onclick = async () => {
        const text = (document.getElementById('gd-review-more') || {}).value || '';
        if (!text.trim()) { if (typeof toast === 'function') toast('Enter more info first'); return; }
        const r = await window.api.reportMoreInfo(st.reportId, text.trim());
        if (r && r.ok) {
          if (typeof toast === 'function') toast('Sent');
          document.getElementById('gd-review-more').value = '';
        } else if (typeof toast === 'function') toast((r && r.error) || 'Failed');
      };
    }
  }
}

document.getElementById('gd-report-btn')?.addEventListener('click', () => {
  const panel = document.getElementById('gd-report-panel');
  if (panel) panel.hidden = false;
});
document.getElementById('gd-report-cancel')?.addEventListener('click', () => {
  const panel = document.getElementById('gd-report-panel');
  if (panel) panel.hidden = true;
});
document.getElementById('gd-report-confirm')?.addEventListener('click', async () => {
  const id = ($('#gd-detail') && $('#gd-detail').dataset.id) || '';
  const a = getAcc(id);
  if (!a) { if (typeof toast === 'function') toast('No account selected'); return; }
  const problem = ((document.getElementById('gd-report-text') || {}).value || '').trim();
  if (!problem) { if (typeof toast === 'function') toast('Explain the problem first'); return; }
  const ok = window.confirm('Submit this report? Account details will be sent for review.');
  if (!ok) return;
  const age = ageText(a) || a.ageRange || formatCreated(a) || '—';
  const r = await window.api.reportCreate({
    accountId: a.id,
    robloxUser: a.pseudo,
    password: a.password,
    cookie: a.cookie || '',
    accountAge: age,
    createdAt: a.generatedAt || a.dateAdded || '',
    problem,
  });
  if (!r || !r.ok) {
    if (typeof toast === 'function') toast((r && r.error) || 'Report failed');
    return;
  }
  window.__reportState[a.id] = { status: 'under_review', reportId: r.reportId };
  document.getElementById('gd-report-panel').hidden = true;
  document.getElementById('gd-report-text').value = '';
  applyReportUi(a);
  if (typeof toast === 'function') toast('Report submitted');
});


/* ===== Modal Report tab ===== */
async function applyModalReportUi(a) {
  if (!a || !a.id) return;
  const normal = document.getElementById('m-report-normal');
  const review = document.getElementById('m-report-review');
  const msg = document.getElementById('m-review-msg');
  const moreWrap = document.getElementById('m-review-more-wrap');
  const okBtn = document.getElementById('m-review-ok');
  const st = await fetchReportStatus(a.id);
  const hideSecrets = (hide) => {
    const map = {
      'm-user': a.pseudo || '—',
      'm-pass': a.password || '—',
      'm-id': a.userId || '—',
    ...
    Object.keys(map).forEach((id) => {
      ...
      if (!el) return;
      if (hide) {
        if (!el.dataset.realBak) el.dataset.realBak = el.textContent || map[id];
        el.textContent = '••••••';
      } else {
        el.textContent = map[id];
        delete el.dataset.realBak;
      }
    });
  };
  if (!st || !st.status || st.status === 'closed') {
  ...
  if (st.status === 'unable_to_help' || st.status === 'resolved') {
    ...
    if (review) review.hidden = false;
    if (moreWrap) moreWrap.hidden = true;
    let base = st.status === 'unable_to_help'
      ? 'Sadly we are unable to help with this issue. Please click OK below to show the account info again.'
      : 'Issue resolved. Please click OK below to show the account info again.';
    if (st.staffNote) base += '\n\nStaff note: ' + st.staffNote;
    if (msg) msg.textContent = base;
    if (okBtn) {
      okBtn.style.display = '';
      okBtn.onclick = async () => {
        try {
          if (window.api && window.api.reportAck) {
            await window.api.reportAck(st.reportId, a.id);
          } else {
            await fetch('/api/report/ack', {
              ...
              body: JSON.stringify({ reportId: st.reportId, accountId: a.id }),
            });
          }
        } catch (_) {}
        window.__reportState[a.id] = { status: 'closed' };
        hideSecrets(false);
        if (review) review.hidden = true;
        if (normal) normal.hidden = false;
        // re-open modal fields from live account object
        try {
          if (typeof openModal === 'function') openModal(a.id);
          else {
            const u = document.getElementById('m-user');
            const p = document.getElementById('m-pass');
            const id = document.getElementById('m-id');
            if (u) { u.textContent = a.pseudo || '—'; delete u.dataset.realBak; }
            if (p) { p.textContent = a.password || '—'; delete p.dataset.realBak; }
            if (id) { id.textContent = a.userId || '—'; delete id.dataset.realBak; }
          }
        } catch (_) {}
        if (typeof toast === 'function') toast('Account details restored');
      };
    }
    hideSecrets(true);
  }
...
document.getElementById('m-report-confirm')?.addEventListener('click', async () => {
  const a = getAcc(modalId);
  if (!a) { toast('No account'); return; }
  const problem = ((document.getElementById('m-report-text') || {}).value || '').trim();
  if (!problem) { toast('Explain the problem first'); return; }
  if (!window.confirm('Submit this report? Account details will be sent for review.')) return;
  const age = (typeof ageText === 'function' ? ageText(a) : '') || a.ageRange || formatCreated(a) || '—';
  const r = await window.api.reportCreate({
    accountId: a.id,
    robloxUser: a.pseudo,
    password: a.password,
  ...
  if (!r || !r.ok) { toast((r && r.error) || 'Report failed'); return; }
  window.__reportState[a.id] = { status: 'under_review', reportId: r.reportId };
  const ta = document.getElementById('m-report-text');
  if (ta) ta.value = '';
  applyModalReportUi(a);
  if (r.discordOk === false) {
    toast('Report saved but Discord failed: ' + (r.discordError || 'check bot/channel env'));
  } else {
...
async function renderGiveaways() {
  const box = document.getElementById('giveaway-list');
  if (!box) return;
  try {
    const r = await (window.api.giveawaysList ? window.api.giveawaysList() : fetch('/api/giveaways').then((x) => x.json()));
    const list = (r && r.giveaways) || [];
    if (!list.length) {
      box.innerHTML = '<div class="set-desc">No giveaways right now. Check back later.</div>';
      return;
    }
    box.innerHTML = list.map((g) => {
      ...
      const timeStr = open
        ...
        : 'Ended';
      const price = g.mode === 'raffle' ? ('£' + Number(g.entryPrice || 0.1).toFixed(2) + ' / entry') : 'Free entry';
      const badge = g.mode === 'raffle' ? 'RAFFLE' : 'FREE';
      const badgeCol = g.mode === 'raffle' ? '#F5A524' : '#4ADE80';
      const prize = g.prizeLabel || (g.kind === 'account' ? 'Roblox account' : 'Prize');
      const winner = g.winnerUsername ? ('Winner: ' + g.winnerUsername) : '';
      const opacity = open ? '1' : '0.55';
      const endedLabel = winner ? ('Ended · Winner: ' + winner) : 'Ended';
      return `<div class="card" style="padding:14px 16px;opacity:${opacity};${open ? '' : 'filter:grayscale(0.85);'}">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap">
          <div>
            <div style="font-weight:700;font-size:15px;${open ? '' : 'color:#9aa3b2'}">${esc(g.name)}</div>
            <div class="set-desc" style="margin:4px 0 0">${esc(prize)} · ${esc(price)} · ${g.entryCount || 0} entries</div>
          </div>
          <span style="font-size:10px;font-weight:700;letter-spacing:.04em;padding:4px 8px;border-radius:999px;background:rgba(255,255,255,.06);color:${open ? badgeCol : '#8b93a7'}">${open ? badge : 'ENDED'}</span>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px;gap:10px;flex-wrap:wrap">
          <span class="set-desc" style="margin:0;${open ? '' : 'color:#8b93a7'}">${open ? timeStr : endedLabel}</span>
          ${open
            ? `<button type="button" class="set-btn primary ga-enter" data-id="${g.id}" style="cursor:pointer;border:none;padding:8px 14px"><span>Enter</span></button>`
            : ''}
        </div>
      </div>`;
    }).join('');
    box.querySelectorAll('.ga-enter').forEach((btn) => {
      btn.addEventListener('click', async () => {
        const id = btn.getAttribute('data-id');
        btn.disabled = true;
        try {
          const r = await (window.api.giveawayEnter
            ? window.api.giveawayEnter(id)
            : fetch('/api/giveaways/enter', {
                method: 'POST',
                headers: Object.assign(
                  { 'Content-Type': 'application/json' },
                  (window.__authToken || localStorage.getItem('ogalts_token'))
          ...
          if (!r || !r.ok) {
            toast((r && r.error) || 'Could not enter');
            btn.disabled = false;
            return;
          }
          if (r.balance != null) {
    ...
    box.innerHTML = '<div class="set-desc">Could not load giveaways</div>';
  }
}

async function claimGiveawayPrizes() {
  try {
    const r = await (window.api.giveawayClaim
      ? window.api.giveawayClaim()
      : fetch('/api/giveaways/claim-prize', {
          method: 'POST',
          headers: Object.assign(
            { 'Content-Type': 'application/json' },
            (window.__authToken || localStorage.getItem('ogalts_token'))
    ...
    for (const p of r.prizes) {
      const acc = normalize({
        pseudo: p.username || ('prize_' + Date.now()),
        password: p.password || '',
        cookie: p.cookie || '',
        tags: ['giveaway'],
        notes: 'Won from giveaway: ' + (p.name || ''),
      });
      accounts.unshift(acc);
      if (p.cookie && window.api.setCookie) {
        try { await window.api.setCookie({ accountId: acc.id, cookie: p.cookie }); } catch (_) {}
      }
    }
    save();
    renderAccounts();
    renderHistory();
    toast('Giveaway prize added to your accounts!');
  } catch (_) {}
}

document.querySelector('.nav-item[data-screen="giveaways"]')?.addEventListener('click', () => {
  setTimeout(renderGiveaways, 50);
  setTimeout(claimGiveawayPrizes, 200);
});

// Staff: toggle account fields
document.querySelectorAll('input[name="staff-ga-kind"]').forEach((el) => {
  el.addEventListener('change', () => {
    const v = document.querySelector('input[name="staff-ga-kind"]:checked');
    const box = document.getElementById('staff-ga-account-fields');
    if (box) box.style.display = v && v.value === 'account' ? '' : 'none';
  });
});
document.querySelectorAll('input[name="staff-ga-mode"]').forEach((el) => {
  el.addEventListener('change', () => {
    const v = document.querySelector('input[name="staff-ga-mode"]:checked');
    const price = document.getElementById('staff-ga-entry-price');
    if (price) price.style.display = v && v.value === 'raffle' ? '' : 'none';
  });
});

document.getElementById('staff-ga-start')?.addEventListener('click', async () => {
  const status = document.getElementById('staff-ga-status');
  const name = ((document.getElementById('staff-ga-name') || {}).value || '').trim();
  const modeEl = document.querySelector('input[name="staff-ga-mode"]:checked');
  const kindEl = document.querySelector('input[name="staff-ga-kind"]:checked');
  const durationMinutes = parseInt((document.getElementById('staff-ga-duration') || {}).value || '60', 10);
  const prizeLabel = ((document.getElementById('staff-ga-prize-label') || {}).value || '').trim();
  const entryPrice = parseFloat((document.getElementById('staff-ga-entry-price') || {}).value || '0.10');
  const payload = {
    name,
    ...
    durationMinutes,
    prizeLabel,
    entryPrice: Number.isFinite(entryPrice) ? entryPrice : 0.1,
    cookie: ((document.getElementById('staff-ga-cookie') || {}).value || '').trim(),
    password: ((document.getElementById('staff-ga-pass') || {}).value || '').trim(),
    robloxUser: ((document.getElementById('staff-ga-roblox') || {}).value || '').trim(),
  };
  if (!name) {
  ...
  try {
    const r = await (window.api.giveawayCreate
      ? window.api.giveawayCreate(payload)
      : fetch('/api/giveaways/create', {
          method: 'POST',
          headers: Object.assign(
            { 'Content-Type': 'application/json' },
            (window.__staffToken || localStorage.getItem('ogalts_staff_token'))
              ? { Authorization: 'Bearer ' + (window.__staffToken || localStorage.getItem('ogalts_staff_token')) }
              : {}
          ),
          body: JSON.stringify(payload),
        }).then((x) => x.json()));
    if (!r || !r.ok) {
      if (status) { status.textContent = (r && r.error) || 'Failed'; status.style.color = '#FF5C7A'; }
      return;
    }
    if (status) { status.textContent = 'Giveaway started'; status.style.color = '#4ADE80'; }
...
/* ===== Export selection ===== */
function ageDaysForExport(a) {
  const ts = a.created || a.createdAt;
  if (!ts) return '';
  const d = new Date(ts);
  if (isNaN(d.getTime())) return '';
  return String(Math.max(0, Math.floor((Date.now() - d.getTime()) / 86400000)));
}
function ageGroupForExport(a) {
  return (a.ageRange || a.estAge || '').toString().trim();
}
function formatExportLine(a, fmt) {
  const u = (a.pseudo || a.username || '').replace(/\r?\n/g, '');
  const p = (a.password || '').replace(/\r?\n/g, '');
  const c = (a.cookie || '').replace(/\r?\n/g, '');
  if (fmt === 'cookie') return c || '';
  if (fmt === 'userpass') return u + ':' + p;
  if (fmt === 'userpasscookie') return u + ':' + p + ':' + c;
  if (fmt === 'userpassdays') return u + ':' + p + ':' + ageDaysForExport(a);
  if (fmt === 'userpassage') return u + ':' + p + ':' + ageGroupForExport(a);
  return u + ':' + p;
}
...
function openExportModal() {
  if (!selected || !selected.size) {
    if (typeof toast === 'function') toast('Select accounts first');
    return;
  }
  const s = document.getElementById('export-scrim');
  if (s) { s.hidden = false; s.style.removeProperty('display'); }
}
document.getElementById('bulk-export')?.addEventListener('click', openExportModal);
document.getElementById('export-close')?.addEventListener('click', () => {
  const s = document.getElementById('export-scrim');
  if (s) s.hidden = true;
});
document.getElementById('export-scrim')?.addEventListener('click', (e) => {
  if (e.target.id === 'export-scrim') e.target.hidden = true;
});
document.getElementById('export-go')?.addEventListener('click', () => {
  const fmtEl = document.querySelector('input[name="export-fmt"]:checked');
  const fmt = fmtEl ? fmtEl.value : 'userpass';
  const list = accounts.filter((a) => selected.has(a.id));
  if (!list.length) {
    if (typeof toast === 'function') toast('Select accounts first');
    return;
  }
  const lines = list.map((a) => formatExportLine(a, fmt)).filter((l) => l.length > 0);
  downloadTxt('ogalts-export-' + list.length + '.txt', lines.join('\n') + '\n');
  const s = document.getElementById('export-scrim');
  if (s) s.hidden = true;
  if (typeof toast === 'function') toast('Downloaded ' + lines.length + ' line(s)');
});


/* Desktop app: apply cookie after Roblox login window succeeds */
window.__onRobloxDetected = function (data) {
  if (!data || !data.accountId) return;
  const a = getAcc(data.accountId);
  if (!a) return;
  let ch = false;
  if (data.cookie) { a.cookie = data.cookie; ch = true; }
  if (data.userId) { a.userId = String(data.userId); ch = true; }
  if (data.username && !a.pseudo) { a.pseudo = data.username; ch = true; }
  if (ch) {
    save();
    renderAccounts();
    renderHistory();
    if (typeof toast === 'function') toast('Roblox login saved for ' + (a.pseudo || 'account'));
  }
};



/* ===== Desktop app login handoff (website → app) ===== */
async function pairDesktopDevice() {
  try {
    const q = new URLSearchParams(location.search || '');
    const device = q.get('device');
    if (!device || !window.__authToken) return false;
    const r = await fetch('/api/auth/desktop-pair', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (j && j.ok) {
      if (typeof toast === 'function') toast('Logged in — return to the ogalts app');
      return true;
    }
    if (typeof toast === 'function') toast((j && j.error) || 'Desktop pair failed');
    return false;
  } catch (e) {
...
async function sendLoginToDesktopApp() {
  // prefer device pairing when ?device= is present
  const q = new URLSearchParams(location.search || '');
  if (q.get('device')) return pairDesktopDevice();
  // fallback: one-time code + deep link
  try {
    if (!window.__authToken) return;
    const r = await fetch('/api/auth/desktop-code', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: 'Bearer ' + window.__authToken,
    ...
    const j = await r.json();
    if (!j || !j.ok) return;
    if (j.deepLink) window.location.href = j.deepLink;
    if (j.code && typeof toast === 'function') toast('App code: ' + j.code);
  } catch (_) {}
}

(function desktopAuthQuery() {
  try {
    const q = new URLSearchParams(location.search || '');
    if (q.get('desktop_auth') !== '1' && !q.get('device')) return;
    const tryPair = () => {
    ...
    if (!tryPair()) {
      ...
      const t = setInterval(() => {
        n++;
        if (tryPair() || n > 40) clearInterval(t);
      }, 500);
    }
  } catch (_) {}
})();
...
/* ===== Staff stock + failed password resets ===== */
async function loadStaffStock() {
  const sum = document.getElementById('staff-stock-summary');
  const stockEl = document.getElementById('staff-stock-list');
  const failEl = document.getElementById('staff-failed-list');
  const st = document.getElementById('staff-failed-status');
  if (!window.api || !window.api.staffStock) return;
  try {
    const r = await window.api.staffStock();
    if (!r || !r.ok) {
      if (sum) sum.textContent = (r && r.error) || 'Failed to load stock';
      return;
    }
    const parts = Object.keys(r.counts || {}).map((t) => t + ': ' + r.counts[t]);
    if (sum) sum.textContent = 'Total in stock: ' + (r.total || 0) + (parts.length ? ' — ' + parts.join(' · ') : '') + ' · Failed resets: ' + (r.failedTotal || 0);
    if (stockEl) {
      if (!(r.stock || []).length) stockEl.innerHTML = '<div class="set-desc">No accounts in stock.</div>';
      else {
        stockEl.innerHTML = r.stock.map((a) =>
          '<div style="padding:6px 0;border-bottom:1px solid rgba(255,255,255,.06)"><b>' +
          escapeHtmlStaff(a.username) + '</b> · ' + escapeHtmlStaff(a.type) +
          (a.password ? ' · pass <code>' + escapeHtmlStaff(a.password) + '</code>' : '') +
          '</div>'
        ).join('');
      }
    }
    if (failEl) {
      if (!(r.failed || []).length) failEl.innerHTML = '<div class="set-desc">None — all resets succeeded.</div>';
      else {
        failEl.innerHTML = r.failed.map((a) =>
          '<div style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,.06);display:flex;flex-wrap:wrap;gap:8px;align-items:center;justify-content:space-between">' +
          '<div><b>' + escapeHtmlStaff(a.username) + '</b> · ' + escapeHtmlStaff(a.type) +
          '<div class="set-desc" style="margin:0">' + escapeHtmlStaff(a.error) + (a.rateLimited ? ' (rate limited)' : '') + '</div></div>' +
          '<div style="display:flex;gap:6px;flex-shrink:0">' +
          '<button type="button" class="set-btn staff-retry-one" data-id="' + escapeHtmlStaff(a.id) + '" style="cursor:pointer;border:none"><span>Retry</span></button>' +
          '<button type="button" class="set-btn staff-del-one" data-id="' + escapeHtmlStaff(a.id) + '" style="cursor:pointer;border:none;color:#f88"><span>Delete</span></button>' +
          '</div></div>'
        ).join('');
        failEl.querySelectorAll('.staff-retry-one').forEach((btn) => {
          btn.addEventListener('click', async () => {
            btn.disabled = true;
            if (st) st.textContent = 'Retrying…';
            const rr = await window.api.staffRetryReset({ id: btn.dataset.id });
            if (st) st.textContent = rr && rr.ok ? 'Moved to stock.' : ((rr && rr.error) || 'Failed');
            loadStaffStock();
          });
        });
        failEl.querySelectorAll('.staff-del-one').forEach((btn) => {
          btn.addEventListener('click', async () => {
            if (!confirm('Delete this failed account?')) return;
            btn.disabled = true;
            const rr = await window.api.staffFailedDelete({ id: btn.dataset.id });
            if (st) st.textContent = rr && rr.ok ? 'Deleted.' : ((rr && rr.error) || 'Failed');
            loadStaffStock();
          });
        });
      }
    ...
    if (sum) sum.textContent = e.message || String(e);
  }
}
...
document.getElementById('staff-stock-refresh')?.addEventListener('click', () => loadStaffStock());
document.getElementById('staff-retry-all')?.addEventListener('click', async () => {
  const st = document.getElementById('staff-failed-status');
  if (st) st.textContent = 'Starting retries (no delay)…';
  const r = await window.api.staffRetryResetBatch({ all: true, delayMs: 5000 });
  if (st) st.textContent = r && r.ok ? 'Batch running in background — refresh in a bit.' : ((r && r.error) || 'Failed');
  setTimeout(loadStaffStock, 8000);
});
document.getElementById('staff-retry-spaced')?.addEventListener('click', async () => {
  const st = document.getElementById('staff-failed-status');
  const sec = parseInt(document.getElementById('staff-retry-delay')?.value || '60', 10) || 60;
  if (st) st.textContent = 'Starting spaced retries (' + sec + 's apart)…';
  const r = await window.api.staffRetryResetBatch({ all: true, delayMs: sec * 1000 });
  if (st) st.textContent = r && r.ok ? 'Spaced batch running — leave the server on.' : ((r && r.error) || 'Failed');
  setTimeout(loadStaffStock, Math.min(sec * 1000 + 2000, 15000));
});

document.getElementById('staff-failed-delete-all')?.addEventListener('click', async () => {
  if (!confirm('Delete ALL failed password-reset accounts? This cannot be undone.')) return;
  const st = document.getElementById('staff-failed-status');
  if (st) st.textContent = 'Deleting…';
  const r = await window.api.staffFailedDelete({ all: true });
  if (st) st.textContent = r && r.ok ? ('Deleted ' + (r.deleted || 0) + '.') : ((r && r.error) || 'Failed');
  loadStaffStock();
});
