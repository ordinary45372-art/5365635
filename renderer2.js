'use strict';
/* RBXFORGE renderer — wires the pixel-perfect UI to the real app logic.
...
const TYPE_LABEL = { 'alt': 'Standard', '+30 days old': 'Aged · 30d+', '+1 year old': 'Aged · 1y+', '5+ years old': 'Aged · 5y+', 'dump': 'Dump' };
const TYPE_SHORT = { 'alt': 'Standard', '+30 days old': '30d+', '+1 year old': '1y+', '5+ years old': '5y+', 'dump': 'Dump' };
...
async function generateOnce() {
  const keys = keyList();
  if (!keys.length) { toast('Add a Bloxgen API key in Settings'); return; }
...
function createLoop() {
  if (!keyList().length) { toast('Add a Bloxgen API key first'); return; }
...
async function importBloxgen() {
  ...
  toast(`Imported ${created.length} account${created.length === 1 ? '' : 's'} from Bloxgen`);
  ...
  if (t) t.innerHTML = 'Drop your <b>account_history.txt</b> here';
  bgText = '';
}
...
function closeBloxgenModal() {
  ...
  const t = $('#bg-drop-t'); if (t) t.innerHTML = 'Drop your <b>account_history.txt</b> here';
...
$('#ck-refresh-all')?.addEventListener('click', async () => {
  ...
  const conf = await window.api.confirm(`Rotate ${targets.length} cookie${targets.length === 1 ? '' : 's'} to keep them alive? The old cookies become invalid (the app updates them automatically).`, ['Refresh', 'Cancel']);
  if (conf !== 0) return;
...
const CHANGELOG = {
  '1.6.0': [
    ...
    '📥 Import from Bloxgen — drop your account_history .txt (Username:Password:Cookie); it dedupes by username and validates every cookie so none stays unchecked.',
...
function renderKeys() {
  ...
  list.innerHTML = keys.map((k) => `
    <div class="key-row" data-kid="${k.id}">
      ...
      <input class="set-input k-key mono" data-f="key" type="password" value="${escAttr(k.key || '')}" placeholder="Bloxgen API key" />
...
function renderSettings() {
  ...
  window.api.version().then((v) => { $('#set-version').textContent = 'v' + v; $('#set-about-ver').textContent = 'RBXFORGE v' + v; }).catch(() => {});
}
...
function showUnlock() {
  return new Promise((resolve) => {
    ...
    ov.innerHTML = `<div style="width:340px;background:var(--a-surface);border-radius:16px;outline:1px solid var(--a-line);padding:24px;display:flex;flex-direction:column;gap:14px">
      <div style="font-size:16px;font-weight:600;color:var(--a-text)">Unlock RBXFORGE</div>
