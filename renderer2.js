...
async function generateOnce() {
  const keys = keyList();
  if (!keys.length) { toast('Server pool empty or keys not set on host'); return; }
...
function createLoop() {
  if (!keyList().length) { toast('Server pool empty or keys not set on host'); return; }
...
async function refreshAuthUI() {
  ...
  try {
    ...
    const bal = document.getElementById('auth-balance');
    if (bal) bal.textContent = (Number(u.balance) || 0).toFixed(2);
  ...
  if (list && r && r.ok) {
    list.innerHTML = (r.users || []).map((u) =>
      `<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid rgba(255,255,255,.06)">
        <span>${u.username}</span><span>£${(Number(u.balance)||0).toFixed(2)}</span></div>`
...
document.getElementById('staff-bal-go')?.addEventListener('click', async () => {
  ...
  if (typeof toast === 'function') toast(username + ' balance: £' + Number(r.balance).toFixed(2));
