  ...
  try {
    ...
    box.querySelectorAll('.earn-dc-join').forEach((btn) => {
      btn.addEventListener('click', async () => {
        if (!signedIn && !(window.__authToken || localStorage.getItem('ogalts_token'))) {
        ...
        // First try: not confirmed → may return notInServer
        let r = await window.api.earnDiscordCheck({ serverId: id, guildIds: [], confirmed: false });
        if (r && r.ok) {
        ...
        if (r && r.notInServer) {
          // User should join then click again — offer claim confirm on second click
          const ok = window.confirm('Opened Discord invite. After you join the server, click OK to claim your reward.');
          if (!ok) return;
          r = await window.api.earnDiscordCheck({ serverId: id, guildIds: [], confirmed: true });
          if (r && r.ok) {
            if (typeof toast === 'function') toast('+' + '£' + Number(r.credited).toFixed(3) + ' balance');
            const bal = document.getElementById('auth-balance');
            if (bal) bal.textContent = Number(r.balance).toFixed(3);
          } else if (typeof toast === 'function') {
            toast((r && r.error) || 'Could not claim');
          }
          return;
        }
