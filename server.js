  ...
  try {
    ...
    for (const type of BLOXGEN_TYPES) {
      ...
      for (let i = 0; i < need; i++) {
        ...
        pool.push(Object.assign({}, r.data, {
          _pooledAt: Date.now(),
          _poolType: type,
          id: r.data.id || crypto.randomUUID(),
        }));
        await savePool(pool);
        console.log('[pool] added', type, r.data.username || r.data.user || 'account', 'size=', pool.length);
      }
    }
  } catch (e) {
...
app.post('/api/bloxgen/generate', async (req, res) => {
  ...
  if (!SERVER_KEYS.length) {
    return res.json({
      ok: false,
      status: 0,
      error: 'No accounts in pool and BLOXGEN_API_KEYS not set on server. Add keys in Render env.',
    });
  }

  let lastErr = null;
  for (const apiKey of SERVER_KEYS) {
    const r = await bloxgenGenerateOnce(apiKey, genType, region || POOL_REGION || undefined);
    if (r.ok) return res.json({ ok: true, data: r.data, fromPool: false });
    lastErr = r;
    if (r.status === 429) break;
  }
  res.json(lastErr || { ok: false, error: 'Generate failed' });
...
app.post('/api/roblox/change-password', (req, res) => {
  res.json({ ok: false, error: 'Password change not supported in web version for safety' });
});
