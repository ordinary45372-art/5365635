...
const TYPE_LABEL = { 'alt': 'Standard', '+30 days old': 'Aged · 30d+', '+1 year old': 'Aged · 1y+', '5+ years old': 'Aged · 5y+', 'dump': 'Robux accounts' };
const TYPE_SHORT = { 'alt': 'Standard', '+30 days old': '30d+', '+1 year old': '1y+', '5+ years old': '5y+', 'dump': 'Robux' };
// Bloxgen price per 1000 accounts → per-account cost = /1000.
const COST_PER_1K = { 'alt': 0.4, '+30 days old': 0.6, '+1 year old': 0.8, '5+ years old': 1.2, 'dump': 5.0 };
const GEN_UI_PRICE = { 'alt': 0.001, '+30 days old': 0.005, '+1 year old': 0.01, '5+ years old': 0.02, 'dump': 0.04 };
function priceStr(t) { const p = GEN_UI_PRICE[t]; return p != null ? ('£' + p.toFixed(3)) : ''; }
function costStr(type) { const p = COST_PER_1K[type]; return p != null ? '$' + (p / 1000).toFixed(4) : '—'; }
function rawTypeOf(a) { return (a.tags || []).find((t) => COST_PER_1K[t] != null) || null; }
...
function isBloxgen(a) { return (a.tags || []).includes('bloxgen'); }
...
function histRowHTML(a, isNew) {
  return `
    <div class="trow${isNew ? ' is-new' : ''}" data-id="${a.id}">
      ...
      <div class="col-created cell-mono tv-mid">${esc(a.created || a.dateAdded)}</div>
...
function renderAccounts() {
  ...
  body.innerHTML = pageAccts.map((a) => {
    ...
    <div class="at-row${on ? ' sel' : ''}" data-id="${a.id}">
      ...
      <div class="ac-created cell-mono tv-mid" style="font-size:11.5px">${esc(a.created || '—')}</div>
...
function openModal(id) {
  ...
  $('#m-tags').textContent = (a.tags || []).join(', ');
...
function showDetailFromAccount(a, fresh) {
  ...
  $('#gd-sub').textContent = `${typeOf(a)}${a.region ? ' · ' + a.region : ''} · ${fresh ? 'just now' : (a.dateAdded || '')}`;
  $('#gd-id').textContent = a.userId || '—';
  $('#gd-region').textContent = a.region || '—';
  $('#gd-created').textContent = a.created || '—';
...
function addGeneratedAccount(d, opts = {}) {
  ...
  if (settings.autoChangePw && acc.cookie && acc.password) {
...
async function generateOnce() {
  const keys = keyList();
  if (!keys.length) { toast('Sign in with Google or check balance / stock'); return; }
  const candidates = genKeyId === '__all__' ? keys.slice() : [keys.find((k) => k.id === genKeyId) || keys[0]];
  statusPill('generating…'); genMeta('requesting…', '');
  let lastErr = null;
  for (const key of candidates) {
    let r;
    try { r = await window.api.bloxgenGenerate({ apiKey: key.key, type: genType, region: genRegion || undefined }); }
    ...
    if (r.ok) {
      if (settings.caseMode) {
        ...
        statusPill('idle'); genMeta('case opened', 'cost ' + costStr(genType) + '/acct');
        await caseReveal(acc);
        return;
      }
      addGeneratedAccount(r.data);
      statusPill('idle'); genMeta('generated ' + r.data.username, 'cost ' + costStr(genType) + '/acct');
...
function openDropdown(anchor, items, current, onPick) {
  ...
  menu.innerHTML = items.map((it) =>
    `<div class="dd-item${it.value === current ? ' sel' : ''}" data-val="${esc(it.value).replace(/"/g, '&quot;')}"><span>${esc(it.label)}</span>${it.value === current ? '<span class="dd-check">' + CHECK_SVG + '</span>' : ''}</div>`
  ).join('');
  document.body.appendChild(menu);
  const r = anchor.getBoundingClientRect();
  menu.style.left = r.left + 'px';
  menu.style.minWidth = r.width + 'px';
  // Flip upward when there isn't enough room below (e.g. the footer page-size selector).
...
$('#gen-type')?.addEventListener('click', () => {
  openDropdown($('#gen-type'), BLOXGEN_TYPES.map((t) => ({ value: t, label: TYPE_LABEL[t] })), genType, (v) => {
    ...
    $('#gen-meta-right').textContent = 'cost ' + costStr(v) + '/acct';
    renderQuota();
  });
});
...
$('#gen-region')?.addEventListener('click', () => {
  openDropdown($('#gen-region'), REGION_OPTS, genRegion, (v) => {
    genRegion = v; $('#gen-region .select-value').textContent = v || 'Auto';
  });
});
...
function createLoop() {
  if (!keyList().length) { toast('Sign in with Google or check balance / stock'); return; }
...
$('#btn-create-loop')?.addEventListener('click', createLoop);
...
$('.gen-options .ghost-icon')?.addEventListener('click', () => comingSoon('Reset options'));
...
function renderSupport() {
  ...
  set('#sup-platform', (navigator.platform || '—'));
...
function renderSettings() {
  ...
  window.api.version().then((v) => { $('#set-version').textContent = 'v' + v; $('#set-about-ver').textContent = 'ogalts v' + v; }).catch(() => {});
}
...
$('#set-autopw')?.addEventListener('click', () => { settings.autoChangePw = !settings.autoChangePw; setSwitch($('#set-autopw'), settings.autoChangePw); saveSettings(); });
...
async function boot() {
  let res;
  try { res = await window.api.load(); } catch (e) { toast('Load error: ' + e.message); res = { accounts: [] }; }
  ...
  $('#gen-meta-right').textContent = 'cost ' + costStr(genType) + '/acct';
...
async function refreshAuthUI() {
  ...
  try {
    ...
    if (typeof loadAccounts === 'function') {
      ...
      try {
        const list = await window.api.load();
        if (Array.isArray(list) && typeof accounts !== 'undefined') {
          accounts.length = 0;
...
async function renderEarnServers() {
  ...
  const hint = document.getElementById('earn-plan-hint');
  if (!box) return;
  let myPlan = null;
  let earnAmt = null;
  try {
    const me = await window.api.authMe();
    if (me && me.ok && me.user) {
      myPlan = me.user.collabPlan || null;
      const rates = { basic: 0.10, premium: 0.25, elite: 0.50 };
      earnAmt = myPlan && rates[myPlan] != null ? rates[myPlan] : null;
      if (hint) {
        if (!myPlan) hint.innerHTML = 'You need a <b>Collab</b> plan to earn. Buy one under Collab, then open a Discord ticket.';
        else hint.innerHTML = 'Your plan: <b>' + myPlan + '</b> — earn <b>£' + earnAmt.toFixed(2) + '</b> per partner server you join (once each).';
      }
    } else if (hint) {
      hint.textContent = 'Sign in with Google to earn.';
    }
  } catch (_) {}
  try {
    const cfg = await fetch('/api/config/public').then((r) => r.json());
    ...
    if (!servers.length) {
      box.innerHTML = '<div class="set-desc">No partner servers configured (COLLAB_SERVERS env).</div>';
      return;
    }
    box.innerHTML = servers.map((s) => {
      const planTag = (s.plan && s.plan !== 'any') ? (' · ' + s.plan + ' plan') : '';
      const earnLine = earnAmt != null
        ? ('Earn £' + earnAmt.toFixed(2) + ' for joining')
        : 'Earn amount depends on your collab plan';
      return `<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 0;border-bottom:1px solid rgba(255,255,255,.06)">
        <div>
          <b>${s.label || s.id}</b>${planTag}
          <div class="set-desc" style="margin:4px 0 0;color:var(--accent,#6cf)">${earnLine}</div>
        </div>
        <div style="display:flex;gap:6px;flex-shrink:0">
          <div class="set-btn" data-join="${s.invite || ''}"><span>Join Discord</span></div>
          <div class="set-btn primary" data-earn="${s.id}" data-invite="${s.invite || ''}"><span>Verify & earn</span></div>
        </div>
      </div>`;
    }).join('');
    box.querySelectorAll('[data-join]').forEach((el) => {
      el.addEventListener('click', () => {
        const u = el.getAttribute('data-join');
        if (u) window.open(u, '_blank');
        else if (typeof toast === 'function') toast('No invite for this server');
      });
    });
    box.querySelectorAll('[data-earn]').forEach((el) => {
      el.addEventListener('click', async () => {
        if (!myPlan) {
          if (typeof toast === 'function') toast('Buy a Collab plan first');
          return;
        }
        const id = el.getAttribute('data-earn');
        const invite = el.getAttribute('data-invite');
        const r = await window.api.earnDiscordCheck({ serverId: id, guildIds: [] });
        if (r && r.ok) {
          if (typeof toast === 'function') toast('Earned £' + Number(r.credited).toFixed(3));
          const bal = document.getElementById('auth-balance');
          if (bal) bal.textContent = Number(r.balance).toFixed(3);
        } else if (r && r.notInServer) {
          if (typeof toast === 'function') toast('Join the server first — opening invite');
          if (invite) window.open(invite, '_blank');
        } else if (typeof toast === 'function') {
          toast((r && r.error) || 'Earn failed');
        }
      });
    });
  } catch (e) {
...
/* Robust modal close (X + backdrop) — fixes staff / collab buy X bugs */
(function setupModalCloses() {
  ...
  window.__showScrim = show;
  window.__hideScrim = hide;

  function bindScrim(scrimId, closeId) {
    const scrim = document.getElementById(scrimId);
    if (!scrim) return;
    scrim.addEventListener('click', (e) => {
      if (e.target === scrim) hide(scrim);
    });
    ...
    if (closeBtn) {
      ...
      closeBtn.addEventListener('mousedown', handler);
    }
    // Event delegation on scrim for any .ck-modal-close inside
    scrim.addEventListener('click', (e) => {
      if (e.target.closest && e.target.closest('.ck-modal-close')) {
        e.preventDefault();
        e.stopPropagation();
        hide(scrim);
      }
  ...
  bindScrim('staff-scrim', 'staff-close');
  bindScrim('staff-panel-scrim', 'staff-panel-close');
  bindScrim('collab-buy-scrim', 'collab-buy-close');
  bindScrim('bg-import-scrim', 'bg-import-close');
  bindScrim('ck-import-scrim', 'ck-import-close');

  // When opening staff / collab modals, force show
  document.getElementById('sup-staff-login')?.addEventListener('click', () => {
    show(document.getElementById('staff-scrim'));
  });
  document.querySelectorAll('.collab-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      show(document.getElementById('collab-buy-scrim'));
    });
  });
})();
