...
app.get('/api/config/public', (req, res) => {
  res.json({
    ...
    collabServers: parseCollabServers().map((s) => ({ id: s.id, label: s.label, invite: s.invite, plan: s.plan || 'any' })),
  });
});
...
app.post('/api/staff/collab', async (req, res) => {
  ...
  try {
    ...
    res.json({ ok: true, username: uname, collabPlan: p, referralCode: code });
  } catch (e) {
    res.json({ ok: false, error: e.message });
  }
});
...
app.post('/api/earn/discord-check', async (req, res) => {
  try {
    ...
    const me = await getUserDocFromAuth(u);
    if (!me) return res.json({ ok: false, error: 'Sign in first' });
    if (!me.collabPlan || !COLLAB_PLANS[me.collabPlan]) {
      return res.json({ ok: false, error: 'Collab plan required to earn' });
    }
    const { serverId, guildIds } = req.body || {};
    ...
    // Server may be limited to a plan tier
    if (target.plan && target.plan !== 'any' && target.plan !== me.collabPlan) {
      return res.json({
        ok: false,
        error: 'This server is for ' + target.plan + ' collab members (your plan: ' + me.collabPlan + ')',
      });
    }
    const ids = Array.isArray(guildIds) ? guildIds.map(String) : [];
    if (!ids.includes(String(serverId))) {
      return res.json({
        ok: false,
        notInServer: true,
        invite: target.invite,
        earnAmount: COLLAB_PLANS[me.collabPlan].earn,
        error: 'Not in this Discord server',
      });
    }
    const claimed = Array.isArray(me.earnClaimed) ? me.earnClaimed : [];
    if (claimed.includes(String(serverId))) {
      return res.json({ ok: false, error: 'Already earned for this server' });
    }
    const earn = COLLAB_PLANS[me.collabPlan].earn;
    const newBal = Math.round(((Number(me.balance) || 0) + earn) * 1000) / 1000;
    claimed.push(String(serverId));
    const col = await usersCol();
