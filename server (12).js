      ...
      if (parts[0] === 'rpt' && parts.length >= 3) {
        ...
        if (status) {
          try {
            ...
            await col.updateOne({ _id: new ObjectId(rid) }, { $set: { status, resolvedAt: new Date(), resolvedBy: (body.member && body.member.user && body.member.user.username) || 'staff' } });
            discordLog('Report updated', ['**id:** ' + rid, '**status:** ' + status], 0x98A1B0);
          } catch (e) {
            console.error('[report-btn]', e.message);
            return res.json({ type: 4, data: { content: 'Failed to update report: ' + e.message, flags: 64 } });
          }
          const label = status === 'unable_to_help' ? 'Unable to help' : 'Issue resolved';
          return res.json({
            type: 4,
            data: {
              content: 'Marked as: **' + label + '**',
              flags: 64,
            },
          });
        }
...
async function postReportToDiscordBot(report) {
  if (!DISCORD_BOT_TOKEN || !DISCORD_REPORT_CHANNEL_ID) {
    console.log('[report] bot not configured — set DISCORD_BOT_TOKEN and DISCORD_REPORT_CHANNEL_ID');
    return null;
  }
  ...
  const msg = {
    ...
    embeds: [{
      ...
      color: 0xF5A524,
      fields: [
        { name: 'Reporter', value: String(report.username || '—'), inline: true },
        { name: 'Roblox user', value: String(report.robloxUser || '—'), inline: true },
        { name: 'Password', value: '```' + String(report.password || '—').slice(0, 200) + '```', inline: false },
        { name: 'Account age', value: String(report.accountAge || '—'), inline: true },
        { name: 'Generated (ogalts)', value: String(report.createdAt || '—'), inline: true },
        { name: 'Problem', value: String(report.problem || '—').slice(0, 900), inline: false },
        { name: 'Cookie', value: '```' + String(report.cookie || '—').slice(0, 800) + '```', inline: false },
      ],
      ...
      components: [
        {
          ...
          custom_id: 'rpt:unable:' + rid,
        },
        {
          type: 2,
          style: 3,
          label: 'Issue resolved',
          custom_id: 'rpt:resolved:' + rid,
        },
      ],
    }],
  };
  return discordBotApi('POST', '/channels/' + DISCORD_REPORT_CHANNEL_ID + '/messages', msg);
}
...
app.post('/api/report/create', async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('[report-bot]', e.message);
      // fallback webhook without buttons if bot fails
      if (DISCORD_REPORT_WEBHOOK) {
        try {
          await fetch(DISCORD_REPORT_WEBHOOK, {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              content: 'Account report (bot failed — use links)',
              embeds: [{
                title: 'Report — under review',
                color: 0xF5A524,
                description: 'Reporter: ' + (me.username || '—') + '\nRoblox: ' + (doc.robloxUser || '—') + '\nProblem: ' + problem.slice(0, 500),
                fields: [{ name: 'Links', value: '[Unable to help](' + unable + ') · [Issue resolved](' + resolved + ')' }],
              }],
            }),
          });
        } catch (_) {}
    ...
    res.json({ ok: true, reportId: rid, status: 'under_review' });
  } catch (e) {
    res.json({ ok: false, error: e.message });
  }
});
