...
async function fetchReportStatus(accountId) {
  try {
    ...
    if (r && r.ok && r.status) {
      window.__reportState[accountId] = { status: r.status, reportId: r.reportId };
...
async function applyModalReportUi(a) {
  ...
  const hideSecrets = (hide) => {
    document.querySelectorAll('.modal .tab-panel').forEach((p) => {
      if (p.dataset.tab && p.dataset.tab !== 'report') {
        // when under review hide other tab content when viewing them
        p.querySelectorAll('.mv, .minput, .cred-v, #m-cookie-val').forEach(() => {});
      }
    });
    // hide credentials fields values
    ['m-user','m-pass','m-id'].forEach((id) => {
      const el = document.getElementById(id);
      if (!el) return;
      if (hide) {
        if (!el.dataset.realBak) el.dataset.realBak = el.textContent || '';
        el.textContent = '••••••';
      } else if (el.dataset.realBak != null) {
        el.textContent = el.dataset.realBak;
  ...
  if (st.status === 'unable_to_help' || st.status === 'resolved') {
    ...
    const sendMore = document.getElementById('m-review-send-more');
    if (sendMore) {
      sendMore.onclick = async () => {
        const text = (document.getElementById('m-review-more') || {}).value || '';
        if (!text.trim()) { toast('Enter more info first'); return; }
        const r = await window.api.reportMoreInfo(st.reportId, text.trim());
        if (r && r.ok) { toast('Sent'); document.getElementById('m-review-more').value = ''; }
