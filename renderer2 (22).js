...
async function finishGoogleLogin(credential, username) {
  ...
  try {
    ...
    if (q.get('desktop_auth') === '1') setTimeout(sendLoginToDesktopApp, 300);
  } catch (_) {}
  window.__pendingGoogleCred = null;
  const box = document.getElementById('username-pick');
...
async function sendLoginToDesktopApp() {
  try {
    if (!window.__authToken) {
      if (typeof toast === 'function') toast('Sign in on the website first');
      return;
    }
    ...
    if (!j || !j.ok || !j.deepLink) {
      if (typeof toast === 'function') toast((j && j.error) || 'Could not create desktop link');
      return;
    }
    // Try open the app
    window.location.href = j.deepLink;
    if (typeof toast === 'function') {
      toast('Opening app… Code (if needed): ' + j.code);
    }
    try {
      await navigator.clipboard.writeText(j.code);
    } catch (_) {}
    console.log('[desktop handoff code]', j.code);
  } catch (e) {
...
(function desktopAuthQuery() {
  try {
    const q = new URLSearchParams(location.search || '');
    if (q.get('desktop_auth') !== '1') return;
    // Wait for existing session restore then hand off
    const trySend = () => {
      if (window.__authToken) {
        sendLoginToDesktopApp();
        return true;
      }
      return false;
    };
...
// After successful Google login on website, if desktop_auth=1 hand off
const __origSetTokenFlow = null;
