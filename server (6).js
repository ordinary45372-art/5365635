...
app.post('/api/earn/discord-check', async (req, res) => {
  try {
    ...
    // Reward is set by the collab listing (plan on the server), same for every joiner
    let planKey = target.plan && COLLAB_PLANS[target.plan] ? target.plan : 'basic';
    if (target.plan === 'any') planKey = 'basic';
    const earn = (COLLAB_PLANS[planKey] && COLLAB_PLANS[planKey].earn) || 0.1;
    ...
    // Until Discord OAuth is wired, open invite and allow claim when client sends confirmed:true
    const confirmed = !!(req.body && req.body.confirmed);
    if (!confirmed && !ids.includes(String(serverId))) {
      return res.json({
        ok: false,
        notInServer: true,
        invite: target.invite,
        earnAmount: earn,
        error: 'Join the Discord first, then press Claim',
      });
    }
    const newBal = Math.round(((Number(me.balance) || 0) + earn) * 1000) / 1000;
