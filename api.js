...
async function fetchJson(path, body = {}) {
  try {
    const res = await fetch('/api' + path, {
      method: 'POST',
      headers: Object.assign({ 'Content-Type': 'application/json' }, window.__authToken ? { Authorization: 'Bearer ' + window.__authToken } : {}),
...
window.api = {
  setToken: (t) => { window.__authToken = t || ''; if (t) localStorage.setItem('ogalts_token', t); else localStorage.removeItem('ogalts_token'); },
  authGoogle: (credential, username) => fetchJson('/auth/google', { credential, username }),
  authMe: () => fetchJson('/auth/me', {}),
  authStaff: (username, password) => fetchJson('/auth/staff', { username, password }),
  staffUsers: () => fetchJson('/staff/users', {}),
  staffBalance: (payload) => fetchJson('/staff/balance', payload),
  staffCollab: (payload) => fetchJson('/staff/collab', payload),
